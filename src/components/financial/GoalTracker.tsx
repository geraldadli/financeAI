...
const GoalTracker: React.FC<GoalTrackerProps> = ({ goals }) => {
  const getGoalColor = (goal: FinancialGoal) => {
    ...
    // If less than 30 days left and progress < 80%, show as error
    if (daysLeft < 30 && progress < 80) return 'error';
    // If less than 90 days left and progress < 50%, show as warning
    if (daysLeft < 90 && progress < 50) return 'warning';
    // Otherwise success or primary based on progress
  ...
  const formatTimeRemaining = (targetDate: Date) => {
    const today = new Date();
    const timeLeft = new Date(targetDate).getTime() - today.getTime();
    const daysLeft = timeLeft / (1000 * 3600 * 24);
          ...
          <div key={goal.id} className="border-b border-gray-100 pb-4 last:border-0 last:pb-0">
            <div className="flex items-start justify-between mb-2">
              ...
              <div className="text-right">
                <div className="text-teal-600 font-medium">${goal.currentAmount.toLocaleString()}</div>
